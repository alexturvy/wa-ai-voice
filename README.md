...
1. **~~Exemplars~~ — closed for external reports, open for internal registers.** Three published deliverables are in (`references/exemplars.md`) and the house voice §5 now carries observed rates instead of defaults — which mattered: the defaults would have flagged the em-dashes, the triplet closers, and the "not only… but" in the first sentence of the strongest exemplar. Still open: internal register (memo to a colleague, working note, client email — all three exemplars are published external reports), non-ECE practice areas, and house-vs-individual register, which three deliverables from one or two authors can't settle.
2. **The standing director feedback.** The corrections directors repeat are better calibration data than finished memos, because finished memos are already post-correction. Worth collecting as its own input.
